...
```
[{
  "name" : "被签到者的名称，没啥用，用来显示在日志里的",
  "wwroleId": "可以抓包获取 鸣潮的uid 不签到不填空着， 库街区里面可以看到鸣潮的UID，游戏里也能看到",
  "eeeroleId": "可以抓包获取 战双的uid 不签到不填空着",
  "tokenraw": "抓包获取的token，一般在请求头内",
  "userId": "库街区的id 可以抓包获取，可以在库街区个人资料中看到，就是UID",
  "devCode":"设备代码 可以随机生成或者抓包获取，一般在请求头内"",
  "distinct_id": "抓包获取同名字段，一般在请求头内""
}]
```
...
工作流已经配置为每天北京时间早上 9 点半左右随机时间自动运行。你无需手动操作，只需确保仓库中的代码和 Secrets 配置正确即可。
